...
float flashLed[] = {0, .500, .750, 1, 1, 1, .750, .500, 0};
int flashLedLen = (sizeof(flashLed) / sizeof(int));
float ledShortWhisper[] = {0, .125, .125, .500, .500, .750, .750, .750, .750, .900, .900, 1, 1, 1, 1, 1, 1, 1, 1, .900, .900, .750, .750, .750, .750, .500, .500, .125, .125, 0};
...
void walk() {
  ...
  if (timer >= steps[0] && timer <= steps[stepsLen - 1]) {
    if (timer < steps[stepIndex + 1]) {
      ...
      if (isLeft) {
        SoftPWMSet(JAMBE_1, valLed / 4);
        SoftPWMSet(JAMBE_2, 0);
      } else {
        SoftPWMSet(JAMBE_2, valLed / 4);
        SoftPWMSet(JAMBE_1, 0);
      }
    } else {
      timerDelta = 0;
...
void bass() {
  if (timer > startBass && timer <= beatStart) {
    ...
    if (timerDelta > beatNote) {
      ...
      if (isLeft) {
        SoftPWMSet(JAMBE_1, valLed / 4 + valWhisper );
        SoftPWMSet(JAMBE_2, 0);
      } else {
        SoftPWMSet(JAMBE_2, valLed / 4 + valWhisper );
        SoftPWMSet(JAMBE_1, 0);
      }
    }
  } else {
  ...
  if (timer > blablaEndGuitarStart && timer < break2StartEndOpeningCoiff2) {
    ...
    if (timerDelta1 > deltaT) {
      ...
      if (indexOpening < 2) {
        ...
        servo1.write(map(i, 0, 127, 179, 0));                // sets the servo position according to the scaled value
        servo2.write(map(i, 0, 127, 179, 0));                // sets the servo position according to the scaled value
        servo3.write(map(i, 0, 127, 0, 179));                // sets the servo position according to the scaled value
        servo4.write(map(i, 0, 127, 0, 179));                // sets the servo position according to the scaled value
